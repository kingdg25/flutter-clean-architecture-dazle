...
class UpdateListingUseCase
    extends UseCase<UpdateListingUseCaseResponse, UpdateListingUseCaseParams> {
  final DataListingRepository dataListingRepository;
  UpdateListingUseCase(this.dataListingRepository);

  @override
  Future<Stream<UpdateListingUseCaseResponse>> buildUseCaseStream(
      UpdateListingUseCaseParams? params) async {
    final controller = StreamController<UpdateListingUseCaseResponse>();

    try {
      final listing = await dataListingRepository.update(data: params!.data);

      logger.finest('Update Listing successful.');
      controller.add(UpdateListingUseCaseResponse(listing));
      controller.close();
    } catch (e) {
      logger.severe('Update Listing fail.');
      // Trigger .onError
